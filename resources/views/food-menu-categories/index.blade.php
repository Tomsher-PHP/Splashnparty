...
<div class="d-flex justify-content-between align-items-center mb-24">
    ...
    <div class="d-flex align-items-center gap-2">
        ...
        @can('create_food_menu_categories')
        <a href="{{ route('food-menu-categories.create') }}"
...
<div class="card border-0 shadow-sm">
    <div class="card-header">
        <form method="GET" action="{{ route('food-menu-categories.index') }}">
            ...
            <table class="table bordered-table mb-0">
                <thead class="bg-light">
                    <tr>
                        ...
                        <th>Sort Order</th>
                        <th>Status</th>
                        @if (auth()->user()?->can('edit_food_menu_categories') || auth()->user()?->can('delete_food_menu_categories'))
                    ...
                    <tr>
                        ...
                        <td class="text-end pe-4">
                            <div class="d-flex justify-content-end align-items-center gap-2">
                                @can('edit_food_menu_categories')
                                <a href="{{ route('food-menu-categories.edit', $item) }}"
                                ...
                                @endcan
                                @can('delete_food_menu_categories')
                                <form action="{{ route('food-menu-categories.destroy', $item) }}" method="POST"
                                    class="delete-form">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit"
                                        class="food-category-icon-btn remove-item-btn bg-danger-focus bg-hover-danger-200 text-danger-600 fw-medium w-32-px h-32-px d-flex justify-content-center align-items-center rounded-circle"
