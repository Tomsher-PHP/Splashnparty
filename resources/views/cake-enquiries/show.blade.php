...
<div class="d-flex flex-wrap justify-content-between align-items-center mb-24 gap-3">
    <div>
        <h6 class="mb-4 fw-bold text-neutral-900 dark:text-white">Cake Enquiry Details</h6>
        <p class="mb-0 text-xs text-secondary-light dark:text-neutral-400">View detailed payload and customer messages captured from the cake forms.</p>
    </div>
    <div class="d-flex align-items-center gap-2">
        <a href="{{ route('cake-enquiries.index') }}" class="btn btn-outline-secondary btn-sm d-inline-flex align-items-center gap-2">
            <i class="ri-arrow-left-line"></i> Back to Enquiries
        </a>
        @can('delete_cake_enquiries')
            <form action="{{ route('cake-enquiries.destroy', $cakeEnquiry) }}" method="POST" class="delete-form d-inline-block">
                @csrf
                @method('DELETE')
                <button type="submit" class="btn btn-danger btn-sm d-inline-flex align-items-center gap-2 border-0"
                        data-confirm-title="Delete Cake Enquiry"
                        data-confirm-message="Are you sure you want to permanently delete this cake enquiry?">
        ...
        <div class="card border-0 shadow-sm h-100">
            <div class="card-body p-28">
                <div class="mb-24 pb-20 border-bottom border-neutral-100 dark:border-neutral-800">
                    ...
                    @if($cakeEnquiry->cake)
                        <h6 class="text-xl fw-bold text-neutral-900 dark:text-white mb-8">
                            Enquiry for: {{ $cakeEnquiry->cake->title }} 
                            <a href="{{ route('cakes.edit', $cakeEnquiry->cake->id) }}" class="text-sm fw-medium text-primary-600 dark:text-primary-400 text-decoration-none ms-8" target="_blank" title="View Cake Details">
                                <i class="ri-external-link-line"></i> [Code: {{ $cakeEnquiry->cake->product_code }}]
                            </a>
                        </h6>
                    @else
                        <h6 class="text-xl fw-bold text-neutral-900 dark:text-white mb-8">General Cake Enquiry</h6>
                    @endif
                    <div class="d-flex align-items-center gap-2 text-xs text-secondary-light dark:text-neutral-400">
                        ...
                        <span>Received on {{ $cakeEnquiry->created_at?->format('l, d M Y - h:i A') ?? 'N/A' }}</span>
                    </div>
                </div>
                ...
                <div class="message-display-box p-24 bg-neutral-50 dark:bg-neutral-900 bg-opacity-40 rounded-16 border border-neutral-100 border-opacity-50 dark:border-neutral-800 dark:border-opacity-50">
                    <p style="white-space: pre-wrap;" class="text-neutral-800 dark:text-neutral-200 mb-0">{{ $cakeEnquiry->message }}</p>
                </div>
            </div>
        </div>
    </div>
                ...
                <div>
                    ...
                    <div class="text-center mb-24 pb-24 border-bottom border-neutral-100 dark:border-neutral-800">
                        <div class="w-50-px h-50-px bg-primary-100 dark:bg-primary-950 text-primary-600 dark:text-primary-400 rounded-circle d-flex justify-content-center align-items-center font-bold text-xl mx-auto mb-16 shadow-sm border border-primary-200 border-opacity-35">
                            {{ strtoupper(substr($cakeEnquiry->name ?? 'U', 0, 1)) }}
                        </div>
                        <h6 class="fw-bold text-neutral-900 dark:text-white mb-4">{{ $cakeEnquiry->name }}</h6>
                        <span class="text-xs text-secondary-light dark:text-neutral-400">Cake Enquirer</span>
                    </div>
                    ...
                    <div class="d-flex flex-column gap-20 mb-28">
                        <div>
                            ...
                            <a href="mailto:{{ $cakeEnquiry->email }}" class="text-sm fw-semibold text-primary-600 dark:text-primary-400 text-decoration-none d-inline-flex align-items-center gap-2">
                        ...
                        <div>
                            ...
                            <a href="tel:{{ $cakeEnquiry->phone }}" class="text-sm fw-semibold text-primary-600 dark:text-primary-400 text-decoration-none d-inline-flex align-items-center gap-2">
                        ...
                        <div>
                            <span class="text-2xs text-secondary-light dark:text-neutral-400 d-block mb-4">PREFERRED DATE</span>
                            <span class="text-sm fw-semibold text-neutral-800 dark:text-neutral-200 d-inline-flex align-items-center gap-2">
                                <i class="ri-calendar-line text-md text-primary-600"></i>
                                {{ $cakeEnquiry->preferred_date ? date('l, d M Y', strtotime($cakeEnquiry->preferred_date)) : 'Not Specified' }}
                            </span>
                        </div>
                    </div>
                </div>
