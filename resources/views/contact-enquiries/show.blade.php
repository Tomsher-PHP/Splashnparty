...
<div class="d-flex flex-wrap justify-content-between align-items-center mb-24 gap-3">
    <div>
        <h6 class="mb-4 fw-bold text-neutral-900 dark:text-white">Enquiry Details</h6>
        <p class="mb-0 text-xs text-secondary-light dark:text-neutral-400">View detailed payload and customer messages captured from the website form.</p>
    </div>
    <div class="d-flex align-items-center gap-2">
        <a href="{{ route('contact-enquiries.index') }}" class="btn btn-outline-secondary btn-sm d-inline-flex align-items-center gap-2">
            <i class="ri-arrow-left-line"></i> Back to Enquiries
        </a>
        @can('delete_contact_enquiries')
            <form action="{{ route('contact-enquiries.destroy', $contactEnquiry) }}" method="POST" class="delete-form d-inline-block">
                @csrf
                @method('DELETE')
                <button type="submit" class="btn btn-danger btn-sm d-inline-flex align-items-center gap-2 border-0"
                        data-confirm-title="Delete Enquiry"
            ...
            <div class="card-body p-28">
                <div class="mb-24 pb-20 border-bottom border-neutral-100 dark:border-neutral-800">
                    <span class="badge bg-primary-50 dark:bg-primary-950 text-primary-600 dark:text-primary-400 text-3xs px-12 py-6 rounded fw-bold border border-primary-100 border-opacity-20 mb-12">
                        {{ strtoupper($contactEnquiry->about) }} ENQUIRY
                    </span>
                    <h6 class="text-xl fw-bold text-neutral-900 dark:text-white mb-8">{{ $contactEnquiry->subject }}</h6>
                    <div class="d-flex align-items-center gap-2 text-xs text-secondary-light dark:text-neutral-400">
                        ...
                        <span>Received on {{ $contactEnquiry->created_at?->format('l, d M Y - h:i A') ?? 'N/A' }}</span>
                    </div>
                </div>
                ...
                <div>
                    ...
                    <div class="text-center mb-24 pb-24 border-bottom border-neutral-100 dark:border-neutral-800">
                        <div class="w-50-px h-50-px bg-primary-100 dark:bg-primary-950 text-primary-600 dark:text-primary-400 rounded-circle d-flex justify-content-center align-items-center font-bold text-xl mx-auto mb-16 shadow-sm border border-primary-200 border-opacity-35">
                            {{ strtoupper(substr($contactEnquiry->full_name ?? 'U', 0, 1)) }}
                        </div>
                        <h6 class="fw-bold text-neutral-900 dark:text-white mb-4">{{ $contactEnquiry->full_name }}</h6>
                        <span class="text-xs text-secondary-light dark:text-neutral-400">Website Enquirer</span>
                    </div>
                    ...
                    <div class="d-flex flex-column gap-20 mb-28">
                        <div>
                            ...
                            <a href="mailto:{{ $contactEnquiry->email }}" class="text-sm fw-semibold text-primary-600 dark:text-primary-400 text-decoration-none d-inline-flex align-items-center gap-2">
                        ...
                        <div>
                            ...
                            <a href="tel:{{ $contactEnquiry->phone }}" class="text-sm fw-semibold text-primary-600 dark:text-primary-400 text-decoration-none d-inline-flex align-items-center gap-2">
                        ...
                        <div>
                            ...
                            <span class="text-sm fw-semibold text-neutral-800 dark:text-neutral-200 d-inline-flex align-items-center gap-2">
                                ...
                                {{ $contactEnquiry->preferred_date ? date('l, d M Y', strtotime($contactEnquiry->preferred_date)) : 'Not Specified' }}
                            </span>
                        </div>
                    </div>
                </div>

                {{-- <!-- Instant Reply Action -->
                <div class="pt-20 border-top border-neutral-100 dark:border-neutral-800">
                    <a href="mailto:{{ $contactEnquiry->email }}?subject=RE: {{ rawurlencode($contactEnquiry->subject) }}"
                       class="btn btn-primary-600 btn-sm w-100 d-flex align-items-center justify-content-center gap-2 py-10 rounded-10 fw-bold">
